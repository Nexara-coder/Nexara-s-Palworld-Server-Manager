...
def format_new_value(new_value: str) -> str:
    """
    Formats a value for a key that doesn't exist in the file yet, so
    there's no existing raw value to copy quoting style from -- infers
    it straight from the new value's own shape instead (True/False and
    numbers stay unquoted, everything else gets quoted like a string).
    """
    v = new_value.strip()
    if v in ("True", "False"):
        return v
